...
class PolarStringRepresentation:
    ...
    def from_complex_vector(vector: np.ndarray, decimals: int = 4) -> str:
        """
        Returns 3-phase vector (numpy 1-column array) of complex numbers as
...
def complex_quantity_to_polar_string(qty: Quantity, decimals: int = 4) -> str:
    return PolarStringRepresentation.from_complex_quantity(qty, decimals)

def complex_number_to_polar_string(number: complex, decimals: int = 4) -> str:
    return PolarStringRepresentation.from_complex_number(number, decimals)

def complex_vector_to_string(vector: np.ndarray, decimals: int = 4) -> str:
    return PolarStringRepresentation.from_complex_vector(vector, decimals)
