...
from ..network import Network, REF_NODE_INDEX
...
class UnSymmetricalFault(ABC):
    ...
    def __init__(
        ...
        U_prefault:
            Per-unit network voltage before the fault, i.e. the voltage between
            the reference node of the network and each other network node,
            assuming the network is unloaded before the fault.
        c:
