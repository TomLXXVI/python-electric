...
from ..network import Network, REF_NODE_INDEX, CurrentPath, Branch
...
class ThreePhaseFault:
    ...
    def __init__(
        ...
        U_prefault:
            Nominal system voltage per unit before the fault. It is the
            line-to-neutral voltage between the reference node of the network
            and every other node in the network, assuming that the network is
            unloaded before the fault (neglecting prefault load currents, the
            internal voltage sources of all synchronous machines are equal both
            in magnitude and phase, and can be replaced by one equivalent
            source).
        c:
            Voltage factor according to IEC 60909-0. Default value is 1.0. If
